  ...
  describe 'PUT #update' do
    context 'with valid params' do
      it 'updates the requested house' do
        house = create(:house)
        put :update, params: { id: house.to_param, house: { bio: 'this house is a nightmare...' } }, session: valid_session
        house.reload
      ...
      it 'renders a JSON response with the house' do
        house = create(:house)
        put :update, params: { id: house.to_param, house: {
          type: 'house',
    ...
    context 'with invalid params' do
      it 'renders a JSON response with errors for the house' do
        house = create(:house)
        put :update, params: { id: house.to_param, house: { rent: 'very expensive' } }, session: valid_session
        expect(response).to have_http_status(:unprocessable_entity)
        expect(response.content_type).to eq('application/json')
      end
    end
